...
# Recent Update
2026.09.05 自己正則化を追加しました。トリガーワードの隣に並ぶタグを、モデルが元々そうしていた通りに保ちます
...
### 自己正則化
　キャラクターLoRAはトリガーワードだけを覚えるわけではありません。どのキャプションでも隣に並んでいるタグ、`1girl`や`solo`なども一緒にキャラクターを覚えてしまい、それらだけでLoRAが発火します。通常は正則化画像で対処しますが、これは効果が薄いものです。凍結したモデルにとってもその画像は完全には復元できないので損失に下限があり、LoRAが何も悪いことをしていない時点でも最適化が動き続けるからです。

　`train self reg`を0より大きくすると、代わりにモデル自身の振る舞いを教師にします。同じlatentの同じ点で、キャプションからトリガーワードを取り除いたとき、LoRAの予測は凍結したモデルがそこで出した予測と一致しなければならない、という制約です。用意する画像はなく、LoRAがゼロのとき損失は厳密にゼロなので、ずれていく下限がありません。LoRAが変えるものは全てトリガーワードが担うことになります。

　完全に分離することはできません。キャラクターは`1girl`なので、その領域を全く触らずに学習することは原理的に不可能です。`train self reg`は「キャラクターの再現度」と「他のタグを汚さないこと」の交換レートになります。まず1から試してください。

　ステップは交互に進みます。1ステップはキャラクターの学習、次はキャプションの残りを固定する側です。そのため反復回数は通常の倍程度を設定してください。`train batch size`が2以上であれば`train self reg batched`で同じステップに両方を入れられます。バッチを半分ずつ使うのでメモリは1バッチ分のままで、しかも両者がlatentの全く同じ点を共有するため制約が強くなります。

　キャプションから単語を抜くと、それ以降のタグの位置が全てずれます。するとLoRAが単語ではなく「タグの位置」を手がかりにしてしまうことがあります。`image shuffle tags`はキャプションをシャッフルしてどの位置も当てにならなくし、`train self reg filler`はトリガーワードが抜けた場所に別の語を入れて位置のずれ自体を無くします。どちらも有効にしておく価値があります。
...
> [!NOTE]
> flow matching系のモデル(Z-Image, Anima, Krea2)では`min/max timesteps`はノイズレベルの1000分の1として解釈されるため、同じ数値が同じ位置を指します。ただし効果はSD系より遥かに強く出ます。Animaの差分LoRAは強度1では使えませんが、0.1では同じ方向の通常のスライダーとして機能します。その強度で使うか、`network strength`を0.1程度に設定して強度1で使えるファイルとして保存してください。
...
|image shuffle tags|キャプションのタグの順序をシャッフルする|
|train self reg|自己正則化の重み。0で無効。[自己正則化](#自己正則化)を参照|
|train self reg filler|トリガーワードを抜いた場所に入れる語。以降のタグの位置がずれなくなる|
|train self reg noise|固定する側を学習画像ではなく純ノイズで確かめる割合、0～1。画像が届かないlatentを守る|
|train self reg batched|固定を交互のステップではなく同じステップで行う。バッチサイズ2以上が必要|
